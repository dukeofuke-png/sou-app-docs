...
- **Milestone B: `courses`/`lessons`/`lesson_plans`/`lesson_plan_files` (Course/Lesson/Plan architecture)** — approved schema recorded in Section 5.5.11. **15 Sep 2026: Slice 1 complete** — schema (all four tables + indexes) plus Course/Lesson CRUD+update (`routes/courses.js`, `routes/lessons.js`), tutor-ownership enforcement (identical 404 whether nonexistent or another tutor's), and calendar/format validation (`services/dateValidation.js`), verified end-to-end. **17 Sep 2026: Slice 2 complete** — `plan_json` structural validation (v1 chunk fields frozen: `arrangement_id`, `notes`, `timing_minutes`) plus idempotent `PUT`/`GET /api/lessons/:id/plan`, verified end-to-end, `materials-server` commit `abf0bd2`. `lesson_plan_files`/Prompt Sheet generation remain unimplemented. **17 Sep 2026: Slices 3a–3b complete** — minimal Course/Lesson navigation frontend (`sou-song-browser` `2b97c2f`) plus the full Lesson Plan authoring screen (chunk editor, two-step Song→Arrangement picker reusing `GET /api/songs`/`GET /api/arrangements?song_id=`, move up/down reordering, save/reload round-trip verified byte-exact) (`d51668b`), verified end-to-end against the live backend and confirmed in a real browser. A search-submit defect in the Arrangement picker (no Enter/button action) was found via manual browser testing and fixed (`fb9f343`), with RTL regression coverage and a button-layout fix added (`2d5b3e6`) after a stale-dev-server-bundle false alarm was ruled out. A follow-up UI-refinement pass (`93bf83d`) relabelled fields ("Lesson Activity" numbering, "Song Arrangement (optional)", "Lesson Notes"), added human-readable validation-error mapping and an explicit "No Songs found" state, and confirmed the Move Up/Down boundary-disable logic was already correct (no change needed there, test coverage added). Prompt Sheet generation remains the next unscoped piece, pending Print Key and Lesson Key architecture (see 5.5.12).
...
Applied to WeasyPrint specifically: pinned at implementation time (15 Sep 2026) to `weasyprint==66.0` in `materials-server/requirements.txt` — the exact version `pip install weasyprint` resolved to that day, not left floating. `nixpacks.toml` (new file, `materials-server` root) carries the `aptPkgs` list above plus a `cmds` step for `pip install -r requirements.txt`; this file has been reasoned through and matched against the spike findings but **not yet deployed/exercised against Railway** — flagged as unverified in production until a real deploy confirms it.

#### 5.5.10 Two decisions recorded from implementation (15 Sep 2026), left open
...
#### 5.5.12 Key Model Phase 1: pitch-class chord data + tutor-confirmed transposition origin (implemented 17 Sep 2026)

**Canonical model established this session, superseding any earlier notion of a fixed per-Arrangement key:** a Song Arrangement is key-flexible — it has no product-facing 'Arrangement Key.' The only persisted, tutor-facing key choices going forward are a Print's fixed `print_key` and an optional per-Lesson-Activity `lesson_key` — **neither is built yet**; both are future scope, to be architected before Prompt Sheet generation. A transient 'Current View Key' plus an `auto | flats | sharps` accidental preference exist only as UI state, never persisted. This slice lays the pitch-class foundation those future slices build on; it does not itself implement Print Key or Lesson Key.

**`body_json` schema_version 2** (required for new writes; existing rows eagerly migrated — see below):
- Every chord decomposition gains `root_pc` (integer 0–11, always server-recomputed, never trusted from a client) and nullable `bass_pc` (slash-chord bass, same rule). `symbol`/`root`/`quality`/`bass` are unchanged — spelling is preserved exactly as authored; pitch class is additive.
- New top-level `body_json.transposition_origin`: `{ tonic_pc, tonic_spelling, mode, source, confirmed }`. `source` ∈ `song_data | tutor_entered | legacy_default_teaching_key | sa_inference`. Any field present is validated regardless of `confirmed`; when `confirmed: true`, `tonic_pc`/`tonic_spelling`/`source` are all required. Absent, or `confirmed: false` with everything null, is fully valid.
- Two-tier parser, `services/noteKeyParser.js`: `parseNoteName()` (Tier 1 — a bare note + optional accidental → pitch class + preserved spelling; the only parser chord-root/slash-bass decomposition ever uses) and `parseKeyString()` (Tier 2, built on Tier 1 — a full key string including optional mode, e.g. `'F# minor'`).
- `transposition_ready` — computed on every read, never stored: true only when `transposition_origin.confirmed === true` AND every chord with a non-empty `symbol` has a valid `root_pc` AND every chord with a slash bass has a valid `bass_pc`. An Arrangement with one unrecognised chord symbol remains fully saveable but reports `transposition_ready: false` — never silently claims readiness.
- Source Key suggest-then-confirm UI added to `ArrangementBuilder.js`'s review screen — deliberately not literally beside `default_teaching_key`'s input, which only exists on the paste screen (unmounted before an existing Arrangement can be reopened); a disclosed deviation from literal field adjacency, not an oversight. Pre-populates from new `GET /api/arrangements/song-key-suggestion`, reusing the exact existing `originalKey` waterfall (`key_getsongbpm → original_key (if valid) → key_best`) via newly-shared `dbManager.resolveOriginalKeyCandidate()`. Tutor accepts (`source: 'song_data'`) or overrides with manual text (`source: 'tutor_entered'`) — the frontend never sends a pitch class; the backend parses and derives it from `source_key_text` alone, discarding any client-supplied `tonic_pc` outright.
- `default_teaching_key` is conceptually deprecated but NOT touched at runtime this slice — it still gates publish/generate-PDF exactly as before, unremoved until Print Key selection exists to replace that gate.
- `services/resourceRenderer.js` confirmed already version-agnostic (reads `sections` generically, never checks `schema_version`) — no change needed; v1 and v2 bodies render through the same path permanently. Frozen v1 `resources.composition_json` snapshots are never touched or 'upgraded.'

**Migration — eager, single-transaction, one identified file (17 Sep 2026):** all 3 existing Arrangement rows (ids 7/8/9, all for `50_cent_in_da_club`) migrated to v2 in one transaction against the exact local dev DB file, with a proven forced-failure rollback test (0 rows changed) run first. A raw-copy WAL-checkpoint gap was found and fixed during this process — an un-checkpointed file copy produced a stale backup missing a since-deleted row; checkpoint-before-copy is now required for backups of this DB. Legacy `default_teaching_key` values were carried forward as unconfirmed `transposition_origin` evidence (`source: 'legacy_default_teaching_key'`, `confirmed: false`) where parseable — never auto-confirmed. `resources`/`resource_files` and every other database file in the repo confirmed untouched (direct query + mtime check). The migrated database file itself was not committed.

**Known open data-integrity gap, not fixed this session:** `50_cent_in_da_club.songs.original_key` still holds the historical corrupted value `'193.46'` (see the 20 Aug 2026 mode/key corruption fix) — the resolution waterfall correctly skips it via `key_getsongbpm`, so it doesn't currently produce a wrong suggestion, but the underlying corrupted value remains unfixed at the data layer. Carried forward as a next-session item.

**Verified:** `materials-server` — 5 new test files, 135/136 passing (1 pre-existing unrelated failure, reproduced identically with changes stashed). `sou-song-browser` — `ArrangementBuilder.test.js` (5 new tests) + existing suite, 17/18 passing (same pre-existing `App.test.js` CRA-boilerplate failure). Migration verified row-by-row against a full copy before the real run; idempotency re-run confirmed (0 migrated, 3 skipped).

**Explicitly out of scope this slice:** Print Key UI/storage, Lesson Key, Prompt Sheet generation, any transposition control/UI, AI/SA key inference, Song Canvas implementation.

`materials-server` `20345f6`, `sou-song-browser` `a9d87d2`.
...
### 15 September 2026 — Print/PDF Pipeline Built and Verified End-to-End (Six Slices)

**Scope:** turned the 7 Sep 2026 architecture (Section 5.5.9) into working code across `materials-server` and `sou-song-browser`, one slice at a time, each independently verified against a live backend before moving to the next. See Section 5.5.10 for two decisions recorded along the way (`default_teaching_key` validation status, single-action publish/generate UX).

**Six slices, in order:**
1. **Schema migration** — `resources`/`resource_files` tables + FK indexes added to `dbManager.js`'s `runMigrations()`, guarded/idempotent, verified against both a fresh DB copy and the existing dev DB, including real `UNIQUE` constraint-violation checks. `materials-server` `65908d7`.
2. **Publish endpoint** — `POST /api/arrangements/:id/publish`: gates on `default_teaching_key`, snapshots `body_json.sections` into `resources.composition_json` (deep copy, not a live reference), upserts via `ON CONFLICT (arrangement_id, print_key)` in one transaction. Verified idempotent republish (same row, refreshed snapshot). `materials-server` `5714bf6`.
3. **HTML renderer** — `services/resourceRenderer.js`: pure function, ports `ArrangementBuilder.js`'s chord-row synthesis algorithm server-side so client preview and server-rendered PDF never disagree on column alignment; self-contained HTML (no external network refs, WeasyPrint-safe). `materials-server` `96b5e06`.
4. **WeasyPrint PDF generation (local)** — `services/pdfGenerator.js`: `generatePdfFromHtml()` shells out to `<python> -m weasyprint - -` (stdin/stdout piping, no temp files). `requirements.txt` (new) pins `weasyprint==66.0`; `nixpacks.toml` (new) carries the pip-based install path from Section 5.5.9 — not yet deployed to Railway. `materials-server` `3903af8`.
5. **R2 upload + generate endpoint** — `POST /api/resources/:id/generate-pdf` (new `routes/resources.js`): renders, generates, uploads to R2 at the deterministic key `resource_{id}.pdf` via the same `wrangler r2 object put` invocation `uploadToR2.py` already used, then upserts `resource_files`. Render/generate/upload failures are isolated — nothing is written to `resource_files` unless the R2 upload is confirmed to have actually succeeded. `materials-server` `7b7f769`.
6. **Frontend trigger** — `ArrangementBuilder.js`: `default_teaching_key` input on the paste screen (plain string, omitted from save payloads when empty) plus a single "Publish PDF" button running publish → generate-pdf in one click, with distinct full-success / publish-failure / partial-success ("published, PDF failed") states. Backing field added to `POST`/`PUT /api/arrangements` (`materials-server` `f2e73f1`, `c12eadd`). `sou-song-browser` `793e9ea`.
...
### 17 September 2026 — Milestone B Slices 3a–3b, UI Refinements, and Key Model Phase 1

**Scope:** `materials-server` and `sou-song-browser`. Three pieces of work, each independently verified: the Course/Lesson/Plan authoring frontend (Slices 3a–3b, closing out Milestone B's UI), a follow-up UI-refinement pass on that same screen, and Key Model Phase 1 (pitch-class chord data + tutor-confirmed transposition origin foundation, Section 5.5.12).

**Slice 3a — minimal Course/Lesson navigation frontend.** Read-only Course list → Lesson list navigation, consuming the existing `GET /api/courses`/`GET /api/lessons?course_id=` endpoints from Slice 1. No new backend endpoints. `sou-song-browser` `2b97c2f`.

**Slice 3b — full Lesson Plan authoring screen.** Chunk editor (add/edit/remove/reorder lesson-activity chunks), a two-step Song→Arrangement picker reusing `GET /api/songs` and `GET /api/arrangements?song_id=` (no new backend surface), move up/down reordering with correctly-verified boundary-disable logic, and a save/reload round-trip against `PUT`/`GET /api/lessons/:id/plan` (Slice 2) verified byte-exact against the live backend. `sou-song-browser` `d51668b`. During manual browser verification, a real defect was found in the Arrangement picker's search box — neither pressing Enter nor a button submitted the search — and fixed in `fb9f343`, with RTL regression coverage plus a button-layout correction added in `2d5b3e6` after first ruling out a stale-dev-server-bundle false alarm (confirmed by a hard restart and re-test, not assumed).

**UI-refinement pass (`93bf83d`).** Relabelled fields for tutor clarity ("Lesson Activity" numbering instead of raw chunk indices, "Song Arrangement (optional)", "Lesson Notes"), added human-readable validation-error mapping (raw backend error strings no longer shown verbatim to the tutor), and added an explicit "No Songs found" empty state to the picker. The Move Up/Down boundary-disable logic was re-checked against this pass and confirmed already correct — no behavior change there, but test coverage was added since none previously existed for that logic specifically.

**Key Model Phase 1 (Section 5.5.12).** Established the canonical model: Arrangements are key-flexible, with `print_key`/`lesson_key` deferred to future slices; this slice only adds pitch-class (`root_pc`/`bass_pc`, 0–11) to chord decomposition and a tutor-confirmable `transposition_origin` on `body_json` (schema_version 2), via a new two-tier parser (`services/noteKeyParser.js`) and a suggest-then-confirm Source Key UI on `ArrangementBuilder.js`'s review screen. All 3 existing Arrangement rows (ids 7/8/9, song `50_cent_in_da_club`) were eagerly migrated to schema_version 2 in one transaction, with a proven forced-failure rollback test run first and a WAL-checkpoint gap in raw-copy backups discovered and fixed along the way (an un-checkpointed `cp` produced a stale backup missing a since-deleted row). Legacy `default_teaching_key` values were carried forward as unconfirmed `transposition_origin` evidence only, never auto-confirmed. `default_teaching_key` itself was not touched at runtime and still gates publish/generate-PDF. `services/resourceRenderer.js` confirmed already version-agnostic — no renderer change needed. **Known open gap, not fixed:** `50_cent_in_da_club.songs.original_key` still holds the corrupted historical value `'193.46'`; the resolution waterfall currently skips it via `key_getsongbpm` so it doesn't produce a wrong suggestion today, but the underlying corrupted value itself remains unfixed at the data layer. `materials-server` `20345f6`, `sou-song-browser` `a9d87d2`.

**Verified:** `materials-server` full suite 135/136 passing (1 pre-existing unrelated `test/config.test.js` failure, reproduced identically with all this session's changes stashed, confirming it predates this work). `sou-song-browser` full suite 17/18 passing (1 pre-existing unrelated `App.test.js` CRA-boilerplate failure). Both `CI=false npm run build` runs succeeded with no new errors/warnings. Migration verified row-by-row against a full copy before the real run, then run for real against the actual dev DB file, with idempotency re-confirmed on a second run (0 migrated, 3 skipped).

**Commits, in order:** `2b97c2f`, `d51668b`, `fb9f343`, `2d5b3e6`, `93bf83d` (`sou-song-browser`, Slices 3a/3b + refinements), `20345f6` (`materials-server`, Key Model Phase 1), `a9d87d2` (`sou-song-browser`, Key Model Phase 1 frontend).

**Not done this session:** Print Key UI/storage, Lesson Key, Prompt Sheet generation/`lesson_plan_files`, any transposition control/UI, fixing the `original_key` data corruption noted above, Course status, 1:1 Lessons, Course Plan, SA-assisted planning.
