...
### 5.4 Operational authority boundaries (added 18 Sep 2026)
Following the 17–18 Sep 2026 nested-clone incident (Section 7, Section 9), the division of authority between the three AI collaborators and Matthew is revised as follows.

**Roles:**
- **ChatGPT ("Ch"):** product behaviour, UX, architectural/schema trade-offs, spotting conceptual omissions, and translating Claude's technical reports into plain English for Matthew. Ch may review and weigh in on schema/persistence/migration design, but only against repository evidence Claude has already verified and supplied — Ch does not independently assert repo/codebase state.
- **Claude:** repository-aware implementation lead and sole author of operational instructions to Copilot (anything that reads, writes, moves, clones, deletes, or otherwise touches the filesystem or git state). Any approved architecture/product decision — whoever originated it — is translated into the actual Copilot prompt by Claude, not issued directly by Ch.
- **Copilot:** inspection and implementation, executing exactly what it's given, verifying before and after via real command output.
- **Matthew:** product owner, making consequential product decisions when asked. **Matthew is not a technical supervisor.** The workflow must not depend on him noticing repository, code, schema, or infrastructure anomalies — that is Claude's and Copilot's job. Claude and Copilot must surface verified evidence, automated-test results, and unresolved risks in plain English before asking Matthew for a product decision, not after something has already gone wrong.

**Claude has final authority over anything touching filesystem, git, or repository state — full stop.** This covers a direct instruction from Ch, a suggestion, a recommendation, or a description of what "should" happen next, however it reaches Claude (a live message, a handover document, a prompt Ch drafted for Matthew to relay). Ch has no direct channel to Copilot or to the local machine; everything from Ch reaches Copilot only by passing through Claude first, which is what makes this enforceable rather than aspirational. Before any such content is translated into a Copilot prompt or acted on, Claude verifies current repository/filesystem state independently rather than accepting Ch's account of it — and pushes back explicitly, in plain terms, rather than either executing it as given or silently rewriting it into something safer without flagging that override. This was the specific failure mode in the 17 Sep incident (an unverified assumption about repo layout reached Copilot as an actual clone command) and is now a hard boundary, not a judgment call.

**The operative rule is not "more checking" or "less checking" — both failure modes are on record and both are real:**
- *Insufficient verification before an operational claim* — e.g. Ch's "disregard `sou-app-docs`" instruction and the nested-clone directive, both later shown wrong by a single verification command that should have run first.
- *Excessive verification/ceremony displacing ordinary progress* — e.g. Ch's own admission, in the 17 Sep session transcript, that "the original unnecessary delay came from auditing repositories before starting the specified task," having conflated a repo-validity question with a scope-auditing exercise and repeatedly misdiagnosed the situation as a result.

**The actual dividing line:**
- **Any change to repository topology** — clones, remotes, branches, canonical file locations, moves, deletions, or other untracked filesystem structure — **requires verified current-state evidence gathered and reported *before* the action is taken**, regardless of whether the surrounding task feels like "just coding." This is what the nested-clone incident skipped.
- **Destructive or difficult-to-reverse actions** additionally require explicit approval from Matthew before proceeding, even once current-state evidence is in hand.
- **Routine edits within an already-verified repository** follow implement → test → continue, without added documentation or verification ceremony layered on top.
- **Architecture, schema, migration, security, and data-integrity decisions** get design review before implementation, because mistakes become expensive once real data exists — this is where Ch's review continues to add genuine value and should not be curtailed.
...
- Active frontend repo is `sou-song-browser/`. A stale outer git repo exists at `SOU App/` for frontend purposes — never commit frontend code from there.
...
- **Canonical local clone for `sou-app-docs` is the `SOU App/` root folder itself** (`/Users/matthew/Documents/SOU App`), confirmed via `git remote -v` → `dukeofuke-png/sou-app-docs`, and has been since that repo was created (30 Aug 2026). `sou-app-docs` is the *remote's* name, not a required local subfolder name — **never `git clone` it into a subfolder of itself.** This is a different repo from the frontend one referenced in the bullet above (which is about `sou-song-browser` code, not docs); the same root folder legitimately serves as the docs clone while that separate frontend warning still applies to frontend commits. A nested `SOU App/sou-app-docs/` duplicate clone was accidentally created during the 17 Sep 2026 session, causing a real same-content-different-path confusion (stale copies got uploaded to Project Knowledge from the wrong path); it was diagnosed via `git remote -v`/`git log`/`git fetch`/`diff` on both paths and removed on 18 Sep 2026 once confirmed clean and fully merged into the parent. See the 18 Sep 2026 entry below for the full incident.
