...
### 2.1 Dated-version discipline
Canonical files may exist in multiple dated copies (e.g. `MASTER_ARCHITECTURE [20 Aug 2026].md` vs `MASTER_ARCHITECTURE [30 Aug 2026].md`). **Always locate and use the most recently dated version** — check the file list rather than trusting whichever copy a search happens to surface first. If two versions carry the same date but different content, flag the conflict explicitly rather than silently picking one.
...
### 2.3 September Utility milestones (current working direction) — ⚠️ sequencing conflict, unresolved
- **Milestone A** — native Arrangement object + A4 Print View + persistent PDF publication. A v4 architecture proposal was submitted for approval; no Copilot implementation is authorised until that proposal is explicitly reviewed and approved.
...
**Open conflict, not yet resolved (added 30 Aug 2026):** `PRODUCT_ROADMAP.md` also documents a separate **Phase 0 → Phase 1** sequence — Phase 0 = stabilise Studio/AI tool integrity (build an AI tool-call audit trail, fix the STUDIO-01 raw-JSON-leak bug, ship the agreed system-prompt rework, validate `searchSeedCatalog`/`promoteSongFromSeed` before shipping them), Phase 1 = PDF/lyric text extraction. It is **not yet clear** whether this Phase 0/1 track blocks Milestone A, runs in parallel with it, or has superseded it as the actual next priority. Full detail in `SOU_App_Handover_Summary.md`'s 30 Aug addendum.

**Do not silently pick one.** Before starting any implementation work, confirm the actual current priority with Matthew, or re-read the newest dated `PRODUCT_ROADMAP.md` in full to check whether it explicitly resolves the ordering.
...
2. **Read `MASTER_ARCHITECTURE.md` and `PRODUCT_ROADMAP.md` at their current version** (see 2.1 — check for dated variants).
